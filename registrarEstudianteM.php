                            ...
                            <div class="container-fluid">
                                <div class="row">
                                    <div class="col-xs-12 col-md-10 col-md-offset-1">
                                        <form action="registrosEstudiantesM.php" method="post">
...
?>
                                            ...
                                            <div class="form-group label-floating">
                                              <label class="control-label">Identificación estudiante</label>
...
<?php
try {
  $stmt= $connection->prepare('SELECT * FROM miembros_materia');
  $stmt->execute();
  while($row = $stmt->fetch()) {
?>
<tbody>
<tr>
<td><?php 
$idm = $connection->prepare('SELECT * FROM materia where id='. $row['id_materia']);
$idm->execute();
while ($rows = $idm->fetch()) {
    echo $rows['nombre']; 
}
?></td>
<td>
    <?php 
    $ids=$row['id_estudiante'];
    try {
...
<td><a href="eliminarEstudianteM.php?id=<?php echo $row['id']; ?>" class="btn btn-danger btn-raised btn-xs"><i class="zmdi zmdi-delete"></i></a></td>
  </tr>
  </tbody>
  <?php
  }
